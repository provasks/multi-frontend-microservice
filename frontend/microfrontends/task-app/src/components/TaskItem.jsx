      ...
      <td className="text-center">
        {task.dueDate ? (
          <div className="task-date">
            ...
            {/* Show if this is a default due date (6 hours from assignment) */}
            {(() => {
              const assignedAt = task.assignedAt ? new Date(task.assignedAt) : new Date(task.createdAt);
              const due = new Date(task.dueDate);
              const sixHoursLater = new Date(assignedAt.getTime() + 6 * 60 * 60 * 1000);
              const isDefaultDueDate = Math.abs(due.getTime() - sixHoursLater.getTime()) < 60000; // Within 1 minute
              return isDefaultDueDate && (
                <div className="text-info small">6h from assignment</div>
              );
            })()}
          </div>
        ) : (
